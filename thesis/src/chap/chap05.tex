...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|p{6cm}|}
        ...
        \hline
        总告警数 & 30 条 & 547 条 & 稳态阶段仍有 futex 阻塞事件发生 \\
        \hline
...
初始化阶段（前 30 秒）的告警数量集中分布于模型冷启动时期。进入稳态后，内存布局已确定、线程池已就绪，推理引擎进入稳定循环，但告警并未消失——30 分钟稳态追踪中仍捕获 547 条告警，最大单次阻塞达 22.4 秒，平均阻塞时长 5.2 秒，说明推理引擎在持续运行中 futex 锁竞争问题仍然存在，但相比初始化阶段平均阻塞时长（7.8 秒）有所降低。上述结果表明，HybriChain 能够持续捕获推理引擎在各阶段的行为特征，异常检测的有效性不随运行时间减弱。
