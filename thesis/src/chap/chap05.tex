...
\textbf{并发阻塞峰值（9 条告警，9–10 秒级）。} 在第二 token 生成周期的 9–10 秒区间内，10 条告警几乎同时触发（TID 分布在 41052–45633），涉及两组 worker 线程（TinyLlama runner 的 TID 41052–41123 与 Qwen runner 的 TID 45624–45633），说明 llama.cpp 在 CPU-only 环境下进行推理时多线程同步机制的结构性瓶颈。

\textbf{对推理 SLA 的影响。} 15.2 秒的单次最大阻塞意味着在最坏情况下，一次 token 生成循环中的单次锁等待可阻塞推理线程十余秒。对于要求低延迟响应的交互式推理场景，用户感知延迟将从理论最优值增加；在高并发场景下，多个线程的锁竞争可能形成级联阻塞效应，导致整体吞吐量有所下降。
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|p{6cm}|}
        ...
        \hline
        时间窗口 & 0–30 秒 & 30–1800 秒 & 初始化含模型加载，稳态为纯推理 \\
        \hline
        总告警数 & 30 条 & 约 8 条 & 稳态阶段告警密度显著降低 \\
        \hline
        最大单次阻塞 & 15.2 秒 & 约 2.1 秒 & 稳态阶段最大阻塞大幅缩短 \\
        \hline
        平均阻塞时长 & 7.8 秒 & 约 0.8 秒 & 稳态 futex 等待时间趋于正常 \\
        \hline
        总事件数 & 13,670 条 & 约 820,000 条 & 按时间比例线性外推 \\
        \hline
        每秒事件率 & 约 574 条/秒 & 约 480 条/秒 & 稳态阶段事件率稳定且略低 \\
        \hline
        内存占用 (RSS) & 峰值 4.8GB & 稳定在 4.5–4.8GB & 稳态内存无持续增长 \\
        \hline
        llama\_decode 调用次数 & 约 72 次 & 约 4,300 次 & 按时间比例与请求数外推 \\
        \hline
    \end{tabular}
\end{table}

初始化阶段（前 30 秒）的告警数量占全追踪时段的 79\%（30/38 条），而该阶段时长仅占总时长的 1.7\%（30/1800 秒）。这一对比表明，推理引擎的主要异常行为集中发生在模型加载与首次推理的初始化阶段：冷启动时的 mmap 大页分配、线程池初始化与第一次 llama\_decode 迭代会引发显著的 futex 锁竞争和阻塞等待。进入稳态后，内存布局已确定、线程池已就绪，推理引擎进入稳定循环，每秒事件率下降且告警密度大幅降低。

\subsection{稳态阶段调用链密度分析}

图~\ref{fig:steady_callchain} 展示了稳态阶段每分钟 llama\_decode 调用次数的时序分布。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/fig9_stage_distribution.png}
    \caption{稳态阶段每分钟 llama\_decode 调用次数时序分布（Qwen2-0.5B）}
    \label{fig:steady_callchain}
\end{figure}
...
图~\ref{fig:steady_memory} 展示了 30 分钟追踪期间推理引擎的内存占用时序。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/fig1_memory_timeline.png}
    \caption{30 分钟推理引擎内存占用时序（Qwen2-0.5B）}
    \label{fig:steady_memory}
\end{figure}

推理引擎在初始化阶段完成模型加载后，内存占用从 0 快速上升至约 4.5GB 并稳定保持，30 分钟全程未出现持续增长趋势。bpftrace 追踪进程 RSS 稳定在约 4MB，相对推理引擎内存占用可忽略不计。上述结果表明，HybriChain 的追踪机制不会对推理引擎的内存行为产生副作用，推理引擎在长时间运行中内存稳定，不存在内存泄漏迹象。
