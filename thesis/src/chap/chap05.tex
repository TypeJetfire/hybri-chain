...
示例~\ref{ex:confirmed} 列出了缝合输出的三条 Confirmed 边，每条记录包含源节点、目标节点、置信状态与层间跨越关系。

\begin{lstlisting}[caption={Confirmed 缝合边输出（JSON 片段）}, label={ex:confirmed}]
...
示例~\ref{ex:alerts} 展示了前 10 条阻塞时长最长的 futex 告警（按阻塞时长降序）。

\begin{lstlisting}[caption={Top 10 futex 阻塞告警（JSON 片段）}, label={ex:alerts}]
