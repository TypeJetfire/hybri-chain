...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|p{6cm}|}
        ...
        \hline
        总告警数 & 30 条 & 待实测 & 预计显著低于初始化阶段 \\
        \hline
        最大单次阻塞 & 15.2 秒 & 待实测 & 预计显著低于初始化阶段 \\
        \hline
        平均阻塞时长 & 7.8 秒 & 待实测 & 预计显著低于初始化阶段 \\
        \hline
        总事件数 & 13,670 条 & 待实测 & 按时间比例线性外推约 820,000 条 \\
        \hline
        每秒事件率 & 约 574 条/秒 & 待实测 & 稳态阶段预计略低于初始化阶段 \\
        \hline
        内存占用 (RSS) & 峰值 4.8GB & 稳定在 4.5–4.8GB & 稳态内存无持续增长 \\
        \hline
        llama\_decode 调用次数 & 约 72 次 & 待实测 & 按时间比例与请求数线性外推约 4,300 次 \\
        \hline
    \end{tabular}
\end{table}

初始化阶段（前 30 秒）的告警数量集中分布于模型冷启动时期，稳态阶段告警密度预计显著降低。这一对比表明，推理引擎的主要异常行为集中发生在模型加载与首次推理的初始化阶段：冷启动时的 mmap 大页分配、线程池初始化与第一次 llama\_decode 迭代会引发显著的 futex 锁竞争和阻塞等待。进入稳态后，内存布局已确定、线程池已就绪，推理引擎进入稳定循环，告警密度预计大幅降低。
