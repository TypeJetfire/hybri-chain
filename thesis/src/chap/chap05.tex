...
\begin{description}
    \item[CGO 桥接链路]（5 条）：\texttt{\_cgo\_*\_Cfunc\_llama\_*} $\rightarrow$ \texttt{llama\_*} 系列函数；
    \item[采样链路]（3 条）：\texttt{llama\_sampler\_sample} $\leftrightarrow$ \texttt{common\_sampler\_sample}；
    \item[调度计算链路]（5 条）：\texttt{llama\_decode} $\rightarrow$ \texttt{ggml\_backend\_sched\_*} 系列；
    \item[模型加载链路]（4 条）：\texttt{llama\_model\_load\_from\_file} $\rightarrow$ \texttt{ggml\_*} 系列初始化函数；
    \item[批量处理链路]（3 条）：\texttt{llama\_batch\_add} $\rightarrow$ \texttt{common\_batch\_add}。
\end{description}
...
图~\ref{fig:confirmed} 列出了缝合输出的三条 Confirmed 边，每条记录包含源节点、目标节点、置信状态与层间跨越关系。
...
\end{lstlisting}
\caption{Confirmed 缝合边输出（JSON 片段）}
\label{fig:confirmed}
\end{minipage}
\end{figure}
...
前 10 条阻塞时长最长的 futex 告警如图~\ref{fig:alerts} 所示。
...
\end{lstlisting}
\caption{Top 10 futex 阻塞告警（JSON 片段）}
\label{fig:alerts}
\end{minipage}
\end{figure}
...
\begin{description}
    \item[时延开销极小。] eBPF 追踪对推理 TTFT 的增量不超过 6.3\%，对用户体验影响可忽略。
    \item[内存开销极低。] bpftrace 追踪进程 RSS 约 4MB，相对于推理引擎数 GB 内存占用可忽略。
    \item[数据压缩高效。] 五步漏斗式降噪在毫秒级时间内完成，处理速度远快于推理时间。
    \item[语义信息完整。] 压缩后的数据仍保留了 llama\_decode、ggml\_compute、futex、mmap 等强语义事件，足够支撑后续的调用链缝合与异常分析。
    \item[异常检测有效。] 规则引擎识别出 30 条 HIGH 级别 futex 阻塞告警，最大单次阻塞 15.2 秒，说明推理引擎在多线程同步机制上存在可用性风险。
    \item[长时间运行稳定。] 30 分钟连续推理追踪实验表明，初始化阶段告警集中（30 条，7.8 秒平均阻塞），稳态阶段告警持续发生（547 条，5.2 秒平均阻塞），最大单次阻塞达 22.4 秒；内存稳定在 10.0–11.0GB 全程无泄漏，调用链密度保持稳定，说明 HybriChain 在长时间运行场景下行为可靠且持续有效。
\end{description}
