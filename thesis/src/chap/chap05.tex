...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|p{6cm}|}
        ...
        \hline
        每秒事件率 & 约 574 条/秒 & 236.7 条/秒 & 稳态阶段每秒事件率低于初始化阶段 \\
        \hline
        内存占用 (RSS) & 峰值 4.8GB & 稳定在 4.5–4.8GB & 稳态内存无持续增长 \\
        \hline
        llama\_decode 调用次数 & 约 72 次 & 3941 次 & 按时间比例与请求数线性外推 \\
        \hline
    \end{tabular}
\end{table}
...
\begin{enumerate}[(1)]
    ...
    \item \textbf{长时间运行稳定。} 30 分钟连续推理追踪实验表明，初始化阶段告警集中（30 条，7.8 秒平均阻塞），稳态阶段告警持续发生（547 条，5.2 秒平均阻塞），最大单次阻塞达 22.4 秒；内存稳定在 4.5–4.8GB 全程无泄漏，调用链密度保持稳定，说明 HybriChain 在长时间运行场景下行为可靠且持续有效。
\end{enumerate}
